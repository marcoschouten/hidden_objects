...
TOP1_JSONL = (
    SCRIPT_DIR / "opa_train_annotation_pipeline_low_iou_top2.jsonl"
)  # detected entries
...
def build_card_data(
    detections: dict, bg_combo_map: dict, chosen: list[tuple[str, str]]
) -> list[dict]:
    ...
    for i, (bg_id, fg_class) in enumerate(chosen):
        ...
        if (i + 1) % 50 == 0 or i == len(chosen) - 1:
            print(
                f"  [{i + 1}/{len(chosen)}] {bg_id} {fg_class}: green={len(good_bboxes)} red={len(bad_bboxes)}"
            )
...
def main():
    ...
    parser.add_argument(
        "--out",
        default=str(Path(__file__).parent / "bbox_reveal_viz.html"),
